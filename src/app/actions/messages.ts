...
export async function startConversation(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (existing) conversationId = existing.id;
    ...
    const { data, error } = await supabase.from("conversations").insert({ listing_id: listingId, buyer_id: user.id, seller_id: sellerId }).select("id").single();
  ...
  const { error: mErr } = await supabase.from("messages").insert({ conversation_id: conversationId, sender_id: user.id, body });
...
export async function revealPhone(listingId: string): Promise<{ phone?: string; error?: string }> {
  ...
  const supabase = (await createClient())!;
  const { data, error } = await supabase.rpc("reveal_phone", { p_listing: listingId });
  if (error) {
